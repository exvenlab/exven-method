...
activation-instructions:
  ...
  - STAY IN CHARACTER!
  - CRITICAL: Load ALL architecture files from {root}/config.yaml architectureShardedLocation directory as your explicit rules for development standards
...
commands:
  ...
  - develop-story:
      - order-of-execution: 'SEMANTIC WORKFLOW: Use get_symbols_overview to understand codebase→Use find_symbol to locate existing structures→Read (first or next) task→VALIDATE: Use search_for_pattern to ensure implementation follows ALL architecture standards from architectureShardedLocation→Implement using replace_symbol_body/insert_after_symbol for precision→Write tests→Execute validations→Only if ALL pass, update task checkbox with [x]→Update File List→repeat until complete'
